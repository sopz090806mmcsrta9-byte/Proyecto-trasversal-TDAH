...
const Menu = ({ onOpenGuide, onOpenBooks, onOpenGames, onShowLevels }) => {
  ...
  // Paletas de colores para planetas
  ...
  useEffect(() => {
    ...
    return () => space.removeEventListener("click", handleClick);
  }, []);

  // Limpiar planeta + galaxias
  const clearPlanets = () => {
    if (!spaceRef.current) return;
    spaceRef.current.querySelectorAll(".space-object").forEach(el => el.remove());
  };
  ...
  return (
    <div className="menu-container">
      ...
      <div className="horizontal-buttons">
        <button className="menu-btn" onClick={onOpenGuide}>Guías</button>
        <button className="menu-btn" onClick={onOpenBooks}>Libros</button>
        <button className="menu-btn" onClick={onOpenGames}>Juegos</button>
        <button className="menu-btn" onClick={onShowLevels}>Niveles</button>
      </div>

      <button className="menu-btn home-btn" onClick={goBack}>⬅ Regresar</button>
      <button className="menu-btn clear-btn" onClick={clearPlanets}>Limpiar</button>
    </div>
  );
};
